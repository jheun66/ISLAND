...
void Model::ReadMaterial(string file)
{
	...
	wstring folder = Utility::ToWstring(Path::GetDirectoryName(file));
	...
	tinyxml2::XMLElement* matNode = root->FirstChildElement();

	do
	{
		tinyxml2::XMLElement* node = matNode->FirstChildElement();
